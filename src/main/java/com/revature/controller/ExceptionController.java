...
public class ExceptionController implements Controller {
    ...
    private ExceptionHandler<FailedLoginException> failedLoginExceptionHandler = (exception, ctx) -> {
        logger.warn(MessageFormat.format("Login failed. Exception message is: {0}.", exception.getMessage()));
        ctx.status(400);
    ...
    private ExceptionHandler<FailedRegisterException> failedRegisterExceptionHandler = (exception, ctx) -> {
        logger.warn(MessageFormat.format("Register failed. Exception message is: {0}.", exception.getMessage()));
        ctx.status(400);
        ctx.json(exception.getMessage());
    };

    private ExceptionHandler<UserNotFoundException> userNotFoundHandler = (exception, ctx) -> {
        logger.warn(MessageFormat.format("User does not exist. Exception message is: {0}.", exception.getMessage()));
        ctx.status(404);
        ctx.json(exception.getMessage());
    };

    private ExceptionHandler<ReimbursementNotFoundException> reimbursementNotFoundHandler = (exception, ctx) -> {
        logger.warn(MessageFormat.format("Reimbursement does not exist. Exception message is: {0}.", exception.getMessage()));
        ctx.status(404);
        ctx.json(exception.getMessage());
    };

    private ExceptionHandler<IllegalArgumentException> invalidArg = (exception, ctx) -> {
        logger.warn(MessageFormat.format("User attempted to enter invalid data. Exception message is: {0}.", exception.getMessage()));
        ctx.status(400);
        ctx.json(exception.getMessage());
    };
